...
import { formatDate, msToHoursMinutes, durationMs } from '@/lib/utils'
...
// Map strain (0–21) to recovery-style percentage for ScoreZoneBadge reuse
function strainToPercent(strain: number): number {
  return Math.round((strain / 21) * 100)
}

const STRAIN_COLOR = (s: number) => {
  if (s >= 14) return '#dc2626'   // High strain
  if (s >= 10) return '#ca8a04'   // Moderate
  return '#0F6E56'                // Low/recovery
}
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {w.avg_heart_rate && ` · ${w.avg_heart_rate} bpm avg`}
                      </p>
                    </div>
                  </div>
